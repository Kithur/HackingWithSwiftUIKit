...
//  Project21
//
//  Created by Roberto Gutiérrez on 20/03/20.
    ...
    func userNotificationCenter(_ center: UNUserNotificationCenter, didReceive response: UNNotificationResponse, withCompletionHandler completionHandler: @escaping () -> Void) {
        ...
        if let customData = userInfo["customData"] as? String {
            ...
            case UNNotificationDefaultActionIdentifier:
                ...
                let ac = UIAlertController(title: "Phone unlocked", message: "Your phone is unlocked", preferredStyle: .alert)
                ac.addAction(UIAlertAction(title: "Cancel", style: .cancel))
                self.present(ac, animated: true)
                
            case "show":
                print("Show more information...")
                let ac = UIAlertController(title: "More info", message: "Your phone is unlocked", preferredStyle: .alert)
                ac.addAction(UIAlertAction(title: "Cancel", style: .cancel))
                self.present(ac, animated: true)
            case "reminder":
                print("Remind me")
